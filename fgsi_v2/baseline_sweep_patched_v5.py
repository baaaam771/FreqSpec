...
def make_large_mask(size, rng, cov_lo=0.40, cov_hi=0.60):
    ...
    import math
    H = W = size
    # [PATCH 10] bounded rejection sampling: retry whole-mask generation up
    # to 8 times, accept the first draw whose coverage lands inside
    # [cov_lo, cov_hi]; otherwise keep the attempt closest to the range.
    # Deterministic: all attempts consume the same per-image rng stream.
    best_m, best_gap = None, float("inf")
    for _attempt in range(8):
        m = _large_mask_once(size, rng, cov_lo, cov_hi)
        cov = m.mean().item()
        if cov_lo <= cov <= cov_hi:
            return m
        gap = max(cov_lo - cov, cov - cov_hi, 0.0)
        if gap < best_gap:
            best_gap, best_m = gap, m
    return best_m


def _large_mask_once(size, rng, cov_lo, cov_hi):
    import math
    H = W = size
...
def build_manifest(args):
    ...
    if getattr(args, "mask_mode", "train") == "coco_object":
        ...
        def _has_obj(p):
            ...
            if not e:
                return False
            lo, hi = args.obj_area_range
            return any(lo <= a["area"] / (e["width"] * e["height"]) <= hi
                       for a in e["anns"])
        before = len(all_imgs)
...
def _prepare_latents(target, sch, item, args, device):
    ...
    if getattr(args, "mask_mode", "train") == "large":
        ...
        m = (make_coco_object_mask(entry, args.image_size, rng,
                                   min_frac=args.obj_area_range[0],
                                   max_frac=args.obj_area_range[1])
             if entry else None)
...
def get_parser():
    ...
    p.add_argument("--obj_area_range", type=float, nargs=2,
                   default=[0.05, 0.50],
                   help="[PATCH 10] instance area fraction range (of the "
                        "original image) eligible for coco_object masks. "
                        "Recorded coverage supports post-hoc strata "
                        "(e.g. 5-15 / 15-30 / 30-50 percent).")
